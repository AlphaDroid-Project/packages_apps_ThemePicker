...
fun <T> FastScrollBar(
    ...
    val groupedItems =
        items.drop(1).groupBy { labelExtractor(it).firstOrNull()?.uppercaseChar() ?: 'A' }
    ...
    Surface(
        modifier = Modifier.padding(end = 12.dp).width(52.dp).heightIn(max = 600.dp),
        shape = MaterialTheme.shapes.extraLarge,
        color = colors.surface.copy(alpha = 0.98f),
        border = BorderStroke(2.dp, colors.outlineVariant.copy(alpha = 0.4f)),
    ) {
        Column(
            modifier = Modifier.fillMaxHeight().padding(vertical = 12.dp, horizontal = 6.dp),
